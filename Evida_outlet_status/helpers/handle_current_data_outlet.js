  ...
  for (let count = 0; count < list_outlet.length - 1; count++) {
    ...
    if (list_ebox_outlet[ebox_outlet_id]) {
      if (list_ebox_outlet[ebox_outlet_id].outlet_status == 2) {
        if (list_ebox_outlet[ebox_outlet_id].outlet_current == 0) {
          list_ebox_outlet[ebox_outlet_id].outlet_current = Number(outlet_current);
        } else {
          if (Number(outlet_current) - list_ebox_outlet[ebox_outlet_id].outlet_current > 1000
            || Number(outlet_current) - list_ebox_outlet[ebox_outlet_id].outlet_current < -1000) {

...
async function transferAppUserData(tableName, getLastUpdateTime) {
    ...
    try {
        ...
        console.log('Last update time in app_user:', lastUpdateTime);
        ...
        if (res.rows.length > 0) {
            for (const row of res.rows) {
                ...
                const checkRes = await clientDB2.query(checkQuery, [row.user_id]);
                ...
                if (checkRes.rows.length > 0) {
                    ...
                    await clientDB2.query(updateQuery, [row.user_id, ...values]);
...
module.exports = transferAppUserData;
